...
public class UserController {
    ...
    public ResponseEntity<String> transferMoney(@PathVariable Long id, @RequestBody TransferRequest transferRequest) {
        ...
        if (transactionResponse != null) {
            if (transferRequest.getAmount().compareTo(threshold) < 0) {
                return ResponseEntity.ok("Transfer request initiated and automatically approved. Amount is less than " + threshold + ".");
            } else {
                return ResponseEntity.ok("Transfer request initiated. Awaiting manual approval.");
            }
        } else{
    ...
    public ResponseEntity<String> approveTransaction(@PathVariable Long userId,
        ...
        try {
            ...
            if (result.toLowerCase().contains("approved successfully") || result.toLowerCase().contains("rejected successfully")) {
            ...
            logger.error("Unexpected error during transaction approval: {}", e);
